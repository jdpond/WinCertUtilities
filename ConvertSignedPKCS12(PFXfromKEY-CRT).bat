...
FOR /F "usebackq delims=" %%i in (`dir /B/AD`) do (
	if exist "%%i\%%i.private.key"  if exist "%%i\%%i.crt" (
		set /a DirCount += 1
		set v!DirCount!=%%i
		echo !DirCount!^) %%~ni
		if !DirCount! GTR 20 (
...
if not defined DirCount ( 
	echo.
	echo You do not have a valid certificate pair ready for conversion.  You need to have a private key ^(%%name%%\%%name%%.private.key^) and
...
if exist "%CertName%\%CertName%.private.pfx" (
	set /p CertConfirm=The key set "%CertName%\%CertName%.private.pfx" already exists, are you sure you want to overwrite^(y,n^)[n]?:
	if "%CertConfirm%" == "" set CertConfirm=n
	if not "%CertConfirm%" == "y" if not "%CertConfirm%" == "Y" (
		echo You elected NOT to create key set "%CertName%\%CertName%.private.pfx"
		pause
		goto :eof
	)
)
...
%OpenSSLExe% pkcs12 %IncludeChain% -aes256 -export -in "%CertName%\%CertName%.crt" -inkey "%CertName%\%CertName%.private.key" -out "%CertName%\%CertName%.private.pfx" -name "%FriendlyName%"
@echo off
echo.
echo PEM Public/Private 509 Certificate has been converted.  The full public/private PKCS12 certificate is: 
echo 	%CD%\%CertName%\%CertName%.private.pfx
echo.
