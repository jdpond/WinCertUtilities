...
if not defined DirCount ( 
	echo.
	echo You do not have a valid certificate set ready for conversion.  You need to have a full key set ^(%%name%%\%%name%%.private.key^)
	echo.
...
if exist "%CertName%\%CertName%.nopass.key" (
...
%OpenSSLExe% rsa -in "%CertName%\%CertName%.private.key" -out "%CertName%\%CertName%.nopass.key"

echo.
echo The following file has been created:
echo       Private no password RSA (x509) Key - ^>^>^> %CD%\%CertName%\%CertName%.nopass.key ^<^<^<
echo.
pause
