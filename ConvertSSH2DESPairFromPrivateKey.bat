...
echo CCreate an SSH2 keyset(3DES encrypted) from an encrypted 509 key ^(private.key^)
echo.
set DirNames=
set /a DirCount=0
...
:ValidCertName
if exist "%CertName%\private\%CertName%.SSH2.des.key" (
	set /p CertConfirm=Are you sure you want to create a new SSH2 key "%CertName%.SSH2.des.key"^(KEY ALREADY EXISTS^)^(y,n^)[y]?:
) else (
	set /p CertConfirm=Are you sure you want to create a new SSH2 key "%CertName%.SSH2.des.key"^(y,n^)[y]?:
)
...
%OpenSSLExe% rsa -inform PEM -in "%Picked_Name%\private\%Picked_Name%.key" -outform PEM -out "%Picked_Name%\private\%Picked_Name%.SSH2.3des.key"

if not %sshkeygenExe% == "" (
	%sshkeygenExe% -f "%Picked_Name%\private\%Picked_Name%.SSH2.des.key" -y %FriendlyName% >"%CD%\%Picked_Name%\%Picked_Name%.SSH2.3des.pub"
)
...
echo		%CD%\%Picked_Name%\private\%Picked_Name%.SSH2.des.key - Private SSH2 Key
if not %sshkeygenExe% == "" (
	echo		%CD%\%Picked_Name%\%Picked_Name%.SSH2.des.pub - Public RSA Key
) else (
	echo.
